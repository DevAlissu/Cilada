...
const PageSidebar: React.FC<PageSidebarProps> = ({ tela, permissoes }) => {
  ...
  const valorF = menus.map((menu) => ({
    ...menu,
    items: menu.items.filter((item) => {
      // Verifica se o item principal está nas permissões
      if (permissoes && permissoes.includes(item.href)) {
        // Se o item tiver subItems, filtra os subItems
        if (item.subItems && item.subItems.length > 0) {
          item.subItems = item.subItems.filter(
            (subItem) => permissoes && permissoes.includes(subItem.href),
          );
        }

        return true; // Mantém o item principal
      }
      // Se o item não estiver nas permissões, verifica os subItems
      if (item.subItems && item.subItems.length > 0) {
        item.subItems = item.subItems.filter(
          (subItem) => permissoes && permissoes.includes(subItem.href),
        );
        return item.subItems.length > 0; // Mantém o item se houver subItems permitidos
      }
      return false; // Remove o item se não estiver nas permissões e não tiver subItems permitidos
    }),
  }));
