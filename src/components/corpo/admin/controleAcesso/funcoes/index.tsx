...
const FuncaosPage = () => {
  ...
  const handleFuncoes = async () => {
    console.time('Tempo de execução de handleFuncoes');
    try {
    ...
    console.timeEnd('Tempo de execução de handleFuncoes');
  };
  ...
  const headers = ['ID', 'Descrição', 'Sigla', 'Ações'];
            ...
            >
              <div className="py-1" role="menu" aria-orientation="vertical">
                {userPermissions.editar && (
                  <button
                    onClick={() => {
                      setIdFuncao(funcao.id_functions);
