...
import { Perfis, getPerfis, deletarPerfil } from '@/data/perfis/perfis';
...
const PerfisPage = () => {
  ...
  const [perfis, setPerfis] = useState({} as Perfis);
  ...
  const handleConfirmDelete = async (idToDelete: string) => {
    try {
      await deletarPerfil(idToDelete);
  ...
  const rows = perfis.data?.map((perfil) => ({
    perfil: perfil.login_perfil_name,
    qtd_telas: perfil.qtd_telas,
    qtd_usuarios: perfil.qtd_usuarios,
    qtd_funcoes: perfil.qtd_functions,
    action: (
            ...
            >
              ...
              >
                {userPermissions.editar && (
                  <button
                    onClick={() => {
                      setIdPerfil(perfil.login_perfil_name);
                      setEditarOpen(true);
                      closeAllDropdowns();
                    }}
    ...
    <div className="h-full flex flex-col flex-grow bg-white dark:bg-slate-900">
      ...
      {idPerfil?.length && (
        <EditarPerfil
          ...
          login_perfil_name={idPerfil}
          onSuccess={() => {
            setEditarOpen(false);
            handlePerfis();
            setIdPerfil(undefined);
          }}
        />
      )}
