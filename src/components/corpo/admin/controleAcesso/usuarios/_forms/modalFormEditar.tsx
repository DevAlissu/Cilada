...
import { UsuarioEdit, atualizarUsuario } from '@/data/usuarios/usuarios';
...
export default function FormEditarUsuario({
  ...
  const [perfilSelecionadoAntigo, setPerfilSelecionadoAntigo] =
    useState<string>('');
  const [filialSelecionadaAntigo, setFilialSelecionadaAntigo] =
    useState<string>('');
  const [funcoesSelecionadasAntigo, setFuncoesSelecionadasAntigo] = useState<
    Funcao[]
  >([]);
  ...
  const handleAdicionarItem = () => {
    ...
    setItensAdicionados((prevItens) => {
      const perfisAtualizados = prevItens.perfis.map((perfil) => {
        const filialExistente = perfil.filial.some(
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Normalização das funções para garantir que `sigla` e `usadoEm` sejam sempre `string`
  ...
  const handleSunClick = (
    ...
    if (isAtivado) {
      ...
      setPerfilSelecionadoAntigo(perfilName);
      setFilialSelecionadaAntigo(filialName);
      setFuncoesSelecionadasAntigo(funcoesNormalizadas);
    }
  };
  ...
  const isDuplicateItem = (): boolean => {
    return itensAdicionados.perfis.some((perfil) => {
      const filialExistente = perfil.filial.some(
        (filial) => filial.nome_filial === filialSelecionada,
      );

      // Se a filial já existir com um perfil diferente, bloqueia
      if (filialExistente && perfil.perfil_name !== perfilSelecionado) {
        return true;
      }
              ...
              <div className="w-[25%] flex justify-center items-end">
                <button
                  className={`w-full px-4 py-2 rounded ${
                    ...
                    filialSelecionada &&
                    !isDuplicateItem() &&
                    (perfilSelecionado !== perfilSelecionadoAntigo ||
                      filialSelecionada !== filialSelecionadaAntigo ||
                      funcoesSelecionadas !== funcoesSelecionadasAntigo)
