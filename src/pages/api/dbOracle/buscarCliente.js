...
//  SELECT PRVENDA FROM DBCLIEN WHERE CODCLI = '07536'
  const COM_VENDA = await sequelize.query(
    'SELECT  * FROM DBCLIEN WHERE NOME LIKE :nome OR CPFCGC LIKE :cpfcgc OR CODCLI LIKE :codigo',
    {
      replacements: { nome: `${descricao.toUpperCase()}%`, cpfcgc: `${descricao}%`, codigo: `${descricao}%` },
