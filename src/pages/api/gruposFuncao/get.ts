...
export default async function handle(
  ...
  try {
    const query: Prisma.dbgpfuncFindManyArgs = {
    ...
    const [gruposFuncao, count] = await prisma.$transaction([
      prisma.dbgpfunc.findMany(query),
      prisma.dbgpfunc.count({ where: query.where }),
    ]);
