...
export default async function handle(
  ...
  try {
    const query: Prisma.dbcompradoresFindManyArgs = {
    ...
    const [compradores, count] = await prisma.$transaction([
      prisma.dbcompradores.findMany(query),
      prisma.dbcompradores.count({ where: query.where }),
    ]);
