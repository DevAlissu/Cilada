...
export default async function handle(
  ...
  try {
    const query: Prisma.dbgpprodFindManyArgs = {
    ...
    const [gruposProduto, count] = await prisma.$transaction([
      prisma.dbgpprod.findMany(query),
      prisma.dbgpprod.count({ where: query.where }),
    ]);
